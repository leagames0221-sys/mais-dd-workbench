"""Docling DocumentConverter wrapper (T2 Week 1、 ADR-101 + ADR-102 SSoT 実装).
...
Output (data/cache/chunks/<DDP-id>/<doc-id>.jsonl):
  - 各 chunk = ChunkMetadata Pydantic instance (ADR-102 schema)
...
def parse_document(
    ...
    *,
    ...
    """1 ファイルを Docling で convert + chunk metadata list 返却 (ADR-102 順守)."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Docling ingestion wrapper (ADR-101/102)")
